...
if (time() > ($gp['expires_at'] ?? 0)) {
    ...
    header('Location: /deped-lrmds-portal/auth/signin.php?expired=1');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($u['status'] === 'email_pending') {
        ...
        header('Location: /deped-lrmds-portal/auth/signin.php?err=email_pending&resend=' . urlencode($resend));
        exit;
    }
    if ($u['status'] === 'suspended') {
        header('Location: /deped-lrmds-portal/auth/signin.php?err=suspended'); exit;
    }
    if ($u['status'] === 'pending') {
        header('Location: /deped-lrmds-portal/auth/signin.php?err=pending'); exit;
    }
    ...
    if (in_array($u['role'], $totpRoles, true)) {
        if (!$u['totp_enabled'] || !$u['totp_secret']) {
            ...
            header('Location: /deped-lrmds-portal/auth/totp_setup.php'); exit;
        }
        $_SESSION['totp_pending_user_id'] = $u['id'];
        header('Location: /deped-lrmds-portal/auth/totp_verify.php'); exit;
    }
